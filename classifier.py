...
class CLASSIFIER:
    # train_Y is interger 
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, openset_gzsl=False, _nclass_openset=25, netDec=None, dec_size=4096, dec_hidden_size=4096):
        ...
        self.nepoch = _nepoch
        self.nclass = _nclass
        self.nclass_openset = _nclass_openset
        ...
        if generalized:
            ...
            self.acc_seen, self.acc_unseen, self.acc_openset, self.H, self.epoch = self.fit_os()
    ...
    def fit_os(self):
        ...
        for epoch in range(self.nepoch):
            ...
            #nclass_openset = 25 #35-15  25-25
            
            unseen, openset = torch.split(self.unseenclasses, int(self.nclass_openset))
            temp = unseen 
            for i in range(self.nclass_openset):
                i_unseen_label = np.argwhere(self.test_unseen_label == unseen[i]).squeeze()
                if i > 0:
                    i_unseen_label = torch.cat((temp, i_unseen_label), -1)
                temp = i_unseen_label 
                
            for i in range(50 - self.nclass_openset):
                i_openset_label = np.argwhere(self.test_unseen_label == openset[i]).squeeze()
                if i > 0:
                    i_openset_label = torch.cat((temp, i_openset_label), -1)
                temp = i_openset_label 

            unseen_label = self.test_unseen_label[i_unseen_label].squeeze()
            unseen_feature = self.test_unseen_feature[i_unseen_label].squeeze()
            
            openset_label = self.test_unseen_label[i_openset_label].squeeze()
            openset_feature = self.test_unseen_feature[i_openset_label].squeeze()
                       
            acc_unseen = self.val_gzsl(unseen_feature, unseen_label, unseen)
            acc_openset = self.val_gzsl(openset_feature, openset_label, openset)
                   
            
            H = 3 / (1/acc_seen + 1/acc_unseen + 1/acc_openset )
            if H > best_H:
                best_seen = acc_seen
                best_unseen = acc_unseen
                best_openset = acc_openset
                best_H = H               
        return best_seen, best_unseen, best_openset, best_H, epoch
    ...
    def val_gzsl(self, test_X, test_label, target_classes): 
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            if self.cuda:
                with torch.no_grad():
                    inputX = Variable(test_X[start:end].cuda()) #, volatile=True) uyarı
            else:
                with torch.no_grad():
                    inputX = Variable(test_X[start:end]) #, volatile=True) uyarı
